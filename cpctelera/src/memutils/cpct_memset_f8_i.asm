...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;
;; Function: cpct_memset_f8_i
;;
;;    Fills up a complete array in memory setting bytes 2-by-2, in chuncks of 
;; 8 bytes. Size of the array must be multiple of 8.The interruption is reenable
;; at end of each chunck
;;
;; C Definition:
;;    void <cpct_memset_f8_i> (void* *array*, <u16> *value*, <u16> *size*);
;;
;; Warning:
...
;;    * At the end of each chunck, it reenables interrupts, no matter how
...
;;
;;  1 - It saves the value of SP to recover it at the end of the chucnk
...
;; Required memory:
;;    C-binding   - 83 bytes
;;    ASM-binding - 78 bytes
;;
;; Time Measures: 
;; (start code)
;;   Case      |   microSecs (us)  |     CPU Cycles         |
;; ----------------------------------------------------------
;;    Any      | 41 + 39CH + 3CHHH | 164 + 156*CH + 12(CHHH) |
;; ----------------------------------------------------------
;; Asm saving  |      -16          |         -64            |
;; ----------------------------------------------------------
;; (end code)
...
standard_1st_pass:
   ld   a, b         ;; [1] Interchange B and C
   ld   b, c         ;; [1]  to use DJNZ in the inner loop
   ld   c, a         ;; [1]

copyloop:
   di                               ;; [1] Disable interrupts first
   ld   (msf8i_restore_sp + 1), sp  ;; [6] Save SP to recover it later on
   ...
   ld sp, #0008      ;; [3] Set Chunck Size (8)
   or a              ;; [1] Clear Carry
   sbc hl, sp        ;; [2] HL = HL - SP - C (Compute next Chunck start)
