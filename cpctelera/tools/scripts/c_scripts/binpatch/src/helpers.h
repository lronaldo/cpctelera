...
// MACROS
// Safely delete any pointer, setting it to nullptr afterwards
#define SAFEDELETE(V)   if ( (V) ) { delete (V); (V) = nullptr; }
...
// FUNCTION PROTOTYPES FOR EXCEPTION HANDLING
void        error                (const TArgs&& err);
void        invalid_arg          (const TArgs&& err);
void        out_of_range         (const TArgs&& err);
...
uint8_t     to8bitValue          (const std::string& str);
