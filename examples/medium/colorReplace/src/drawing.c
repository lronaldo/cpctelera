...
void DrawStars()
{
	...
	for (i = 0; i < NB_STARS; i++)
	{
		...
		// Set color to change
		cpct_setReplaceColorsM0(15, sColorStar[color]);
		
		// Use different drawing color methods
		if (i < NB_STARS/3)
		{
			// Color and draw masked sprite in video memory
			cpct_drawSpriteMaskedColorizeM0(g_star_trans, pvmem, G_STAR_TRANS_W, G_STAR_TRANS_H);
		}
		else if (i > NB_STARS/3 && i < 2*NB_STARS/3)
		{
			// Color and draw masked aligned sprite in video memory
			cpct_drawSpriteMaskedAlignedColorizeM0(g_square, pvmem, G_SQUARE_W, G_SQUARE_H, gMaskTable);
		}
		else
		{
			// Color and copy masked sprite in temporary array
			u8 circleColor[G_CIRCLE_TRANS_W * G_CIRCLE_TRANS_H * 2];
			cpct_spriteMaskedColorizeM0(g_circle_trans, circleColor, G_CIRCLE_TRANS_W, G_CIRCLE_TRANS_H);
...
void DrawSceneBaloons()
{
	...
	for (i = 0; i < gBaloons.nb; i++)
	{
		...
		{		
			if (color != itBaloon->color) 		// Test if need to change Color of Sprite	
			{
				color = itBaloon->color;
				ColorSprite(color);				// Change colors of ballon
			}
